...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Ocultar/Mostrar enlaces (Lógica CORREGIDA)
    const productsLink = document.getElementById('productsLink');
    const locationLink = document.getElementById('locationLink'); // Para Productor
    const manageLocationsAdminLink = document.getElementById('manageLocationsAdminLink'); // Para Admin
    ...
    if (userRole === 'producer') {
        ...
        if (productsLink) productsLink.style.display = 'block'; // Admin ve "Todos los Productos"
        if (manageLocationsAdminLink) manageLocationsAdminLink.style.display = 'block'; // Admin ve "Gestionar Ubicaciones"
        ...
        // Nota: El admin no ve el botón "Añadir Producto +" (showAddProductFormBtn)
    }
    ...
    function switchSection(sectionId) {
        ...
        // Cargar datos de la sección correspondiente
    ...
    function loadProfileImage(imageUrl) {
        if (imageUrl) {
            currentProfileImage.src = imageUrl.startsWith('http') ? imageUrl : `/${imageUrl}`; // Funciona para Cloudinary y local
    ...
    function loadGallery(images) {
        ...
        if (images && images.length > 0) {
            images.forEach(imageUrl => {
                const imgDisplayUrl = imageUrl.startsWith('http') ? imageUrl : `/${imageUrl}`;
    ...
    if (addProductForm) {
        addProductForm.addEventListener('submit', async (e) => {
            ...
            if (!formData.get('imageFile') || formData.get('imageFile').size === 0) { showToast('La imagen es obligatoria.', 'error'); return; }
    ...
    async function openEditProductModal(productId) {
        try {
            ...
            const imageUrl = product.imageUrl ? (product.imageUrl.startsWith('http') ? product.imageUrl : `/${product.imageUrl}`) : '#'; 
             ...
             try {
                 ...
                 if (response.ok) { 
                     ...
                     if (adminLocationMarker) {
                         adminLocationMap.removeLayer(adminLocationMarker);
    ...
    async function openEditPostModal(postId) {
        try {
            ...
            const editPostImagePreview = document.getElementById('editPostImagePreview');
            const imageUrl = post.imageUrl ? (post.imageUrl.startsWith('http') ? post.imageUrl : `/${post.imageUrl}`) : '#';
